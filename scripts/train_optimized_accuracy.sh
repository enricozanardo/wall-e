...
TRAINING_CMD="cargo run --release --bin Wall-E -- data/tiny_stories_sample.json --model-dim $MODEL_DIM --ff-dim $FF_DIM --heads $HEADS --layers $LAYERS --epochs $NUM_EPOCHS --vocab-size 5000 --min-freq 2 --save-path models/high_accuracy_model.walle --learning-rate 0.0001 --enable-skip --strong-anti-rep --json-format --stories $NUM_STORIES --perf-log true --curriculum-examples $CURRICULUM_EXAMPLES"
...
generate_text() {
  ...
  GENERATE_CMD="cargo run --release --bin Wall-E -- --generate-only --model models/high_accuracy_model.walle --prompt \"$prompt\" --max-tokens 75 --perf-log true"
...
echo "cargo run --release --bin Wall-E -- --generate-only --model models/high_accuracy_model.walle --prompt \"Your prompt here\" --max-tokens 100" | tee -a "$LOG_FILE" 